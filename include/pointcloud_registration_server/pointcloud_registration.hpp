...
template <typename PointType, typename NormalType, typename FeatureType>
PCRegistration<PointType, NormalType, FeatureType>::PCRegistration()
{
...
//   
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::interestPointGeneration(const PCP input_cloud, const PCP interest_point_cloud, int algorithm_type)
{
...
//   
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::featureEstimation(const PCP input_cloud, const FCP feature_cloud, int feature_type)
{
	switch(feature_type)
	{
		case pointcloud_registration_server::registration_service::Request::FEATURE_TYPE_NONE:
		{
			pointToFeature(input_cloud, feature_cloud);
			break;
		}
		case pointcloud_registration_server::registration_service::Request::FEATURE_TYPE_NORM:
		{
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::featureEstimationNormals(const PCP input_cloud, const FCP feature_cloud)
{
	...
	// Perform Computation
	norm_est.compute (*temp_normal_cloud);
	normalToFeature(temp_normal_cloud, feature_cloud);
}
// ---------------------------------------------------------
...
// --------------------------------------------------------------------------------------
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::correspondenceEstimation(const FCP feature_cloud, const FCP correspondence_cloud, int correspondence_type)
{
...
// --------------------------------------------------------------------------------------
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformEstimation(const FCP source_cloud, const FCP target_cloud, const FCP transformed_source, Eigen::Matrix4f transform, int transform_type)
{
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformEstimation(const FCP source_cloud, const FCP target_cloud, const FCP transformed_source, Eigen::Matrix4f transform, Eigen::Matrix4f initial_transform_guess, int transform_type)
{
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformEstimationNDT(const FCP source_cloud, const FCP target_cloud, const FCP transformed_source, Eigen::Matrix4f transform, Eigen::Matrix4f init_guess)
{	
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformEstimationICP(const FCP source_cloud, const FCP target_cloud, const FCP transformed_source, Eigen::Matrix4f transform, Eigen::Matrix4f init_guess)
{	
	// Align
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformNDT(const PCP source_cloud, const PCP target_cloud, Eigen::Matrix4f &final_transform, float epsilon, int max_iterations, float step_size, float resolution) //, Eigen::Something pose_estimate)
{
	// TODO
...
template <typename PointType, typename NormalType, typename FeatureType>
void PCRegistration<PointType, NormalType, FeatureType>::transformICP(const PCP source_cloud, const PCP target_cloud, Eigen::Matrix4f &final_transform, float epsilon, int max_iterations, int ksearch, float max_dist, float alpha[4])
{	
...
/*
template <typename PointType, typename NormalType, typename FeatureType>
bool PCRegistration<PointType, NormalType, FeatureType>::preprocessing(pointcloud_registration_server::registration_service::Request& req, pointcloud_registration_server::registration_service::Response& res, int cloud_index)
{
...
template <typename PointType, typename NormalType, typename FeatureType>
bool PCRegistration<PointType, NormalType, FeatureType>::postprocessing(pointcloud_registration_server::registration_service::Request& req, pointcloud_registration_server::registration_service::Response& res, int cloud_index)
{
...
template <typename PointType, typename NormalType, typename FeatureType>
bool PCRegistration<PointType, NormalType, FeatureType>::normalToFeature(NCP normal_cloud, FCP feature_cloud)
{
...
template <typename PointType, typename NormalType, typename FeatureType>
bool PCRegistration<PointType, NormalType, FeatureType>::pointToFeature(PCP point_cloud, FCP feature_cloud)
{

}

// For now, manually controlling which templates are compiled
// To compare, check out how this is done in PCL
//   There, all implementations are in .hpp files #included in .cpp files
//   Actual class template type declarations can be manually declared one-by-one in the .cpp or all pcl PointT types can be used via a special function call
//template class PCRegistration<pcl::PointXYZ, pcl::Normal, pcl::PointXYZ>;
//template class PCRegistration<pcl::PointXYZRGB, pcl::PointXYZRGBNormal, pcl::PointXYZRGBNormal>;